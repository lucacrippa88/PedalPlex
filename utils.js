  ...
  $(document).on("click", ".edit-btn", function () {
    ...
    if (!pedal.controls || pedal.controls.length === 0) {
      ...
      .then(fullPedalRes => {
        ...
        // --- Mantieni il canEdit originale dall'array iniziale ---
        const originalCanEdit = pedal.canEdit;
        pedal = fullPedal;
        pedal.canEdit = originalCanEdit;

        // Aggiorna l'oggetto nell'array pedals
        const idx = pedals.findIndex(p => p._id === pedal._id);
        if (idx !== -1) pedals[idx] = pedal;
        ...
        // pedal ha già i controlli aggiornati, il codice qui sotto aprirà correttamente il modal
      })
    ...
    Swal.fire({
      ...
      didOpen: () => {
        ...
        $("#duplicateBtn").on("click", () => {
          ...
          newPedal.published = "draft"; // force draft
          Swal.close();
      ...
      if (result.isConfirmed) {
        ...
        .then(data => {
          ...
          if (data.success) {
            ...
            if (idx !== -1) {
              pedals[idx] = { ...updated, _id: data.id || updated._id, _rev: data.rev || updated._rev, canEdit: true };
            }
            ...
            updatePedalCounts();
            Swal.fire({ title: 'Gear saved!', icon: 'success', confirmButtonText: 'OK', customClass: { confirmButton: 'bx--btn bx--btn--primary' }});
          } else {
        ...
        Swal.fire({
          ...
          if (deleteConfirm.isConfirmed) {
            ...
            .then(text => {
              let data;
              try { data = JSON.parse(text); } catch(err) { throw new Error("Invalid JSON response from server."); }
              Swal.hideLoading();
              if (data.success) {
                Swal.fire({ title: 'Deleted!', text: 'The gear has been removed.', icon: 'success', confirmButtonText: 'OK', customClass: { confirmButton: 'bx--btn bx--btn--primary' }})
                  .then(() => {
                    const idx = pedals.findIndex(p => p._id === pedal._id);
