...
function populatePresetDropdownByFolder(folderId, preferredPresetId = null, isNewFolder = false) {
    ...
    // Hide or show dropdown & edit button
    if (filteredPresets.length === 0 || isNewFolder) {
        // For new folders or empty: placeholder
        presetSelect.innerHTML = '';
        const placeholder = document.createElement('option');
        placeholder.value = '';
        placeholder.textContent = '-- No presets --';
        ...
        // ✅ Render the pedalboard with correct row positions even if no preset
        renderFullPedalboard(window.pedalboard?.pedals || []);
        ...
        // ✅ Always render pedalboard first with row info
        renderFullPedalboard(window.pedalboard?.pedals || []);

        // ✅ Then apply preset controls on top
        if (selectedPreset) applyPresetToPedalboard(selectedPreset);
